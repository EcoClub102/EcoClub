...
while ($row2 = mysqli_fetch_array($result)) {
    $opt="SELECT SUM(recharge) as total FROM `recharge` WHERE username='$row2[1]' AND status='Success'";
...
$timestamp=strtotime($row2[5])+60*30;
  $date=date('l jS \ F Y H:i:s A', $timestamp);
    $dataRow = $dataRow."
           <tr class='user' ><td >$row2[1]</td><td>$bonus</td><td>$first</td><td>$date</td></tr>";
