...
if ($action == 'getuserinfo') {
	...
	$bonus = round($sum['total'], 2);
		...
		$opti = "SELECT SUM(amount) as total FROM `intrest` WHERE username='$user'";
	$optresi = $conn->query($opti);
	$sumi = mysqli_fetch_assoc($optresi);
	$abbc = round($sumi['total'], 2);
	...
	array_push($userData, ['notice' => $notice], ['bonus' => $bonus],['days'=>$bonus9],['status'=>$bonus9t],['intrest'=>$abbc]);
	$res['error'] = false;
